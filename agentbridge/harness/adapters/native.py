...
_SPECS = (
    _codex("codex.workspace_read", "permissions.agentbridge-run.filesystem",
           effect="host-file-read", label="Read workspace files",
           surface="host-boundary"),
    _codex("codex.workspace_write", "permissions.agentbridge-run.filesystem",
           effect="host-file-write", label="Edit workspace files",
           risk="critical", surface="host-boundary"),
    _codex("codex.process_exec", "default_permissions", "features.shell_tool",
           "features.unified_exec", "features.code_mode_host.enabled",
           "features.code_mode_host.disable_in_process_fallback",
           effect="host-process-exec", label="Run commands",
           risk="critical", surface="host-boundary"),
    _codex("codex.workspace_binding", "-C", "projects",
           effect="workspace-scope-binding", label="Stay within this workspace",
           risk="critical",
           surface="host-boundary"),
    _codex("codex.provider_prompts", "approval_policy",
           effect="provider-native-approval", label="Use provider approval prompts",
           risk="critical"),
    _codex("codex.session_persistence", "--ephemeral",
           effect="provider-persistent-state", label="Save provider sessions",
           risk="high"),
    _codex("codex.strict_config", "--strict-config",
           effect="provider-config-validation", label="Use validated settings",
           risk="high",
           surface="configuration-source"),
    _codex("codex.project_trust", "projects.*.trust_level",
           effect="trusted-project-authority", label="Trust project configuration",
           risk="critical",
           surface="configuration-source"),
    _codex("codex.safe_overlay", "model_context_window",
           "model_auto_compact_token_limit", "personality", "service_tier",
           effect="reviewed-model-config-overlay", label="Use reviewed model settings",
           risk="medium",
           surface="configuration-source"),
    _codex("codex.network", "permissions.agentbridge-run.network.enabled",
           effect="network-read-or-write", label="Use the network",
           risk="critical", surface="host-boundary"),
    _codex("codex.web_search", "web_search", effect="network-read",
           label="Search or read the web"),
    _codex("codex.agentbridge_mcp", "mcp_servers.ab",
           effect="agentbridge-broker-tools", label="Use AgentBridge tools",
           risk="high",
    ...
    _codex("codex.external_mcp", "--ignore-user-config", "mcp_servers",
           effect="external-tool-access", label="Use external tool servers",
           risk="critical",
           surface="configuration-source"),
    _codex("codex.user_config", "--ignore-user-config",
           effect="provider-config-inheritance", label="Inherit user configuration",
           risk="critical",
           surface="configuration-source"),
    _codex("codex.non_user_config", "config/read.layers",
           effect="provider-config-inheritance",
           label="Inherit project, cloud, system or managed configuration",
           risk="critical", surface="configuration-source"),
    _codex("codex.rules", "--ignore-rules", "project_doc_max_bytes",
           effect="instruction-inheritance", label="Inherit project instructions",
           surface="configuration-source"),
    _codex("codex.invocation_overrides", "--profile", "--add-dir", "--image",
           "--enable", "--disable", "--approve-for-me",
           "--dangerously-bypass-approvals-and-sandbox",
           "--dangerously-bypass-hook-trust", "--remote",
           "--remote-auth-token-env", "--search",
           effect="provider-authority-override", label="Broaden provider launch",
           risk="critical", surface="configuration-source"),
    _codex("codex.apps", "features.apps", "features.tool_suggest",
           effect="external-tool-access",
           label="Use connected apps"),
    _codex("codex.plugins", "features.plugins", "features.remote_plugin",
           "features.plugin_sharing", "features.recommended_plugins",
           effect="external-tool-access",
           label="Use provider plugins"),
    _codex("codex.hooks", "features.hooks", effect="host-process-exec",
           label="Run provider hooks",
           risk="critical"),
    _codex("codex.multi_agent", "features.multi_agent", "features.multi_agent_v2",
           effect="provider-delegation", label="Start provider subagents"),
    _codex("codex.browser", "features.browser_use",
           "features.browser_use_external", "features.browser_use_full_cdp_access",
           "features.in_app_browser", effect="browser-control",
           label="Control a browser", risk="critical"),
    _codex("codex.computer_use", "features.computer_use",
           effect="computer-control", label="Control the computer",
           risk="critical"),
    _codex("codex.image_generation", "features.image_generation",
           effect="external-service-use", label="Generate images"),
    _codex("codex.workspace_dependencies", "features.workspace_dependencies",
           "features.executor_capability_discovery", effect="dependency-install",
           label="Install workspace dependencies",
           risk="critical"),
    _codex("codex.skills", "features.skill_mcp_dependency_install",
           "features.skill_search", "skills.include_instructions",
           "skills.bundled.enabled", "skills.config",
           effect="instruction-or-tool-extension",
           label="Use provider skills", risk="critical",
           surface="configuration-source"),
    _codex("codex.memories", "features.memories", "memories.use_memories",
           effect="provider-persistent-state", label="Use provider memories"),
    _codex("codex.shell_snapshot", "features.shell_snapshot",
           effect="host-environment-read", label="Snapshot the host shell",
           risk="critical", surface="host-boundary"),
    _codex("codex.view_image", "features.view_image",
           effect="host-file-read", label="View local images",
           risk="high", surface="host-boundary"),
    _codex("codex.provider_approvals", "features.guardian_approval",
           "features.guardianv2", "features.request_permissions_tool",
           "features.exec_permission_approvals",
           effect="provider-native-approval", label="Use automatic approvals",
           risk="critical"),
    _codex("codex.elicitation", "features.tool_call_mcp_elicitation",
           "features.auth_elicitation", "features.enable_mcp_apps",
           "features.mcp_2026_07_28", "features.non_prefixed_mcp_tool_names",
           effect="credential-or-tool-elicitation", label="Request external access",
           risk="critical"),
    _codex("codex.goals", "features.goals",
           effect="provider-persistent-state", label="Persist provider goals"),
    _codex("codex.in_app_updates", "features.in_app_updates",
           effect="provider-self-update", label="Update the provider runtime",
           risk="critical"),
    _codex("codex.network_proxy", "features.network_proxy",
           "features.respect_system_proxy", effect="network-policy-mutation",
           label="Use provider network proxies", risk="critical",
           surface="host-boundary"),
    _codex("codex.endpoint_override", "OPENAI_BASE_URL",
           "CODEX_EVERYWHERE_API_KEY", "HTTP_PROXY", "HTTPS_PROXY", "ALL_PROXY",
           "model_provider", "model_providers", "openai_base_url",
           "chatgpt_base_url",
           effect="credential-or-endpoint-redirection",
           label="Override credentials or endpoints", risk="critical",
