...
| `sidebar.js` | `renderSidebar` (imported directly by `chat.js`/`settings.js` — it sits *below* page views, not sideways, so this is the one legal direct cross-module import) | Chat list / settings nav / new-chat form / **in-sidebar new-group builder** (chip tray → search → tap-add list → name step — replaced the old modal), whichever the rail/state selects. The chat list updates **per-row in place** (v0.24.20): when the set/order of chats is unchanged it diffs a per-row signature and touches only changed rows, instead of re-swapping the whole `innerHTML` every poll (which reset scroll + flashed). A full rebuild still runs on a structure change (add/remove/reorder) or when arriving from another sidebar variant; `chat.js`'s `patchChatName` uses this to apply a rename without a transcript rebuild. |
