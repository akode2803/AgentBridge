...
The initial display context copies only usernames, display names, avatar markers
...
storage still reads the current viewer from the shared state: this is identity-
equivalent during synchronous initialization because the operation has checked
the session, state generation and fresh chat viewer immediately before rendering.
This change does not redesign historical asynchronous composer callbacks.
...
Cold startup, missing or stale context, details navigation and unavailable
connectivity keep the state-first fallback. No new endpoint, cache-validation
